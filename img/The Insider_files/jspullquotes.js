...
//		var $capfirst = arrOptions[5];
	...
	// Check that the browser supports the methods used
		if (!document.getElementById || !CSCO_WebVPN["call"](document,"createElement") || !CSCO_WebVPN["call"](document,"appendChild")) return false;
		
		var oElement, oClassName, oPullquote, oPullquoteP, oQuoteContent, i, j, k;
	// Find all span elements
		var arrElements = CSCO_WebVPN["call"](document,"getElementsByTagName")('span');
	...
	// Proceed if current element is pullquote
		...
		// Create the blockquote and p elements
				oPullquote = CSCO_WebVPN["call"](document,"createElement")($qcontainer);
				oPullquoteP = CSCO_WebVPN["call"](document,"createElement")('p');
...
// thx: https://www.engr.uga.edu/adminsite/modules/htmlarea/example-fully-loaded.html
				if ($alttext && oElement.firstChild && oElement.firstChild.nodeType == 8) { // 8 == comment
						...
						CSCO_WebVPN["call"](oPullquoteP,"appendChild")(oAltQuote);
					...
					for(j=0;j<oElement.childNodes.length;j++) { //loop through the children of <span>
						...
						if (oCurrChild.nodeType == 8) {
							...
							( $skiplinks && '#' != CSCO_WebVPN["call"](oCurrChild,"getAttribute")('href').substr(0,1) ) || // external link, or...
							( $skipintlinks && '#' == CSCO_WebVPN["call"](oCurrChild,"getAttribute")('href').substr(0,1) )  || // internal (href="#id") link, or...
							( ! CSCO_WebVPN["call"](oCurrChild,"getAttribute")('href') ) || // no href, or...
							( '' == CSCO_WebVPN["call"](oCurrChild,"getAttribute")('href') ) // href is blank
						) ) {
			...
			// append text to the paragraph node
					CSCO_WebVPN["call"](oPullquote,"appendChild")(oPullquoteP);
			// Insert the blockquote element before the span element's parent element
					CSCO_WebVPN["call"](oElement.parentNode.parentNode,"insertBefore")(oPullquote,oElement.parentNode);
				}
			}
		}
	}, // end function init
...
*/
		...
		if ($new_node.nodeType == 1) {
			if ( CSCO_WebVPN["call"]($new_node,"getAttribute")('name') ) { $new_node.removeAttribute('name') };
			if ( CSCO_WebVPN["call"]($new_node,"getAttribute")('id') ) { $new_node.removeAttribute('id') };
		}
		...
		if ($include_all && $the_node.hasChildNodes()) {
			for(i=0;i<$the_node.childNodes.length;i++) {
				...
				CSCO_WebVPN["call"]($new_node,"appendChild")($child_node);
			}
		}
		return $new_node;
	},
	...
	*/
	...
	ltrim : function($str, $chars) {
		$chars = $chars || "\\s";
		return CSCO_WebVPN["call"]($str,"replace")(new RegExp("^[" + $chars + "]+", "g"), "");
	},
